...
export function createModeratorReply(userMessage: string): ModeratorReply {
  return {
    id: `moderator-reply-${Date.now()}`,
    roleId: "ceo",
    speaker: "裁判长回应",
    title: "基于主持人补充信息的临时复核",
    badge: "Follow-up",
    summary:
      "收到主持人追加条件后，我先给出一版临时响应，后续可继续触发多角色重新评估。",
    content: ` 
 ## 主持人最新补充 
 ...
 这个补充条件会直接影响原本的判断边界，因此不应该把它视为普通备注，而应视为新的决策变量。 

 ### 我的处理建议 
 1. 先把新增条件纳入讨论上下文 
 2. 优先判断它影响的是预算、合规还是品牌预期 
 3. 如影响显著，应要求相关角色重新发言 
 4. 最终裁决需要以更新后的条件为准 
