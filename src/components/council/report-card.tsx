...
export function ReportCard({
  ...
  return (
    <motion.article
      ...
      className={cn(
        "relative overflow-hidden rounded-[32px] border border-white/10 bg-white/[0.045] p-6 shadow-[0_20px_80px_rgba(0,0,0,0.22)] backdrop-blur-2xl",
        isFinal && "border-cyan-300/20 bg-cyan-400/[0.05]"
      )}
    >
      <div
        className={cn(
      ...
      <div className="relative z-10">
        ...
        <div className="prose prose-invert mt-6 max-w-none prose-headings:mb-3 prose-headings:text-white prose-p:text-white/78 prose-li:text-white/75 prose-strong:text-white prose-table:block prose-table:w-full prose-table:overflow-x-auto prose-th:text-white prose-td:text-white/75">
          <ReactMarkdown remarkPlugins={[remarkGfm]}>{content}</ReactMarkdown>
        </div>

        <div className="mt-6 rounded-2xl border border-white/8 bg-black/10">
          <button
            type="button"
          ...
          {expanded ? (
            <div className="border-t border-white/8 px-4 py-4">
              <div className="prose prose-invert max-w-none prose-headings:text-white prose-p:text-white/75 prose-li:text-white/75 prose-strong:text-white prose-table:block prose-table:w-full prose-table:overflow-x-auto prose-th:text-white prose-td:text-white/75">
